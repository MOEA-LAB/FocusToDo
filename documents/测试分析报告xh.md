...
|      预计测试内容      |                         预计测试结果                         |
| :--------------------: | :----------------------------------------------------------: |
|    专注页面计时功能    |        不超出最大时间限制（设置为99），均可以正常计时        |
...
​	在设置的范围内，能够正常完成计时。但是不可设置超出范围的时间，无法自由定义。
...
​	可以自由增加，满足多种符号的输入；但输入过长时，会影响事项列表的显示美观；
...
​	可以完成正常记录，但是当事项名称过长时，会出现记录名称截断的BUG，需要保证事项名称在15个字符以内。
...
​	可以正常更换，昵称过长无法全部显示，头像在允许访问设备存储的情况下，可以自由更换。
...
​	可以正常完成对话，但当生成内容过多时，生成的内容可能会出现超出页面的情况。
